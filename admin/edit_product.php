...
$current_exchange_rate = $exchange_stmt->fetch(PDO::FETCH_COLUMN);
if (!$current_exchange_rate) {
    $current_exchange_rate = 1460; // Fallback rate
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
     if (empty($_POST['name']) || !isset($_POST['price_ngn']) || $_POST['price_ngn'] === '' || empty($_POST['description'])) {
        ...
        $current_images = !empty($product['image']) ? explode(',', $product['image']) : ['default.jpg'];
        $image_names = $current_images; // Start with current images
    ...
    for ($i = 0; $i < $file_count; $i++) {
        ...
        if (isset($_POST['remove_images']) && is_array($_POST['remove_images'])) {
            foreach ($_POST['remove_images'] as $image_to_remove) {
                $index = array_search($image_to_remove, $image_names);
                if ($index !== false) {
                    // Remove from array
                    unset($image_names[$index]);
                    // Optionally delete the file from server
                    $file_path = "../uploads/" . $image_to_remove;
                    if (file_exists($file_path) && $image_to_remove !== 'default.jpg') {
                        unlink($file_path);
                    }
                }
            }
            $image_names = array_values($image_names); // Reindex array
        }
        
        // If all images were removed, add default
...
// Price is already stored in NGN, so we display it directly
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div class="flex items-center justify-between">
            ...
            <a href="exchange_rate" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition text-sm">
                Manage Rates
            </a>
        </div>
    </div>
    ...
    <div class="bg-white shadow-md rounded-lg p-8">
        <form action="edit_product?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                         <div class="col-span-2">
                            <input type="number" name="price_ngn" id="price_ngn" value="<?php echo number_format($price_in_ngn, 2, '.', ''); ?>" step="0.01" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500">
                        </div>
                         <div class="col-span-2">
                            <p class="text-sm text-gray-600">Equivalent in USD: <span id="usd-equivalent" class="font-semibold">$<?php echo number_format($price_in_ngn / $current_exchange_rate, 2); ?></span></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <div class="relative border rounded-lg p-2">
                        <img src="../uploads/<?php echo htmlspecialchars($image); ?>" alt="Product Image <?php echo $index + 1; ?>" class="w-full h-24 object-cover rounded">
                        <div class="mt-2 text-center">
                            <label class="flex items-center justify-center text-sm text-red-600 cursor-pointer">
                                <input type="checkbox" name="remove_images[]" value="<?php echo htmlspecialchars($image); ?>" class="mr-1">
                                Remove
                            </label>
                        </div>
                        <?php if ($index === 0): ?>
                        <span class="absolute top-1 left-1 bg-teal-500 text-white text-xs px-2 py-1 rounded">Main</span>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
                </div>
                <p class="text-xs text-gray-500">Check "Remove" to delete images. The first image is the main product image.</p>
            </div>
            ...
            <div class="border-t pt-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="delivery_days" id="delivery_days" value="<?php echo htmlspecialchars($product['delivery_days'] ?? 3); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    </div>
                </div>
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = function(e) {
                        ...
                        previewDiv.innerHTML = `
                            ...
                            <span class="absolute top-1 right-1 bg-green-600 text-white text-xs px-1 rounded">New</span>
                        `;
